...
/*   Updated: 2022/11/03 00:58:06 by ceatgie          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_pwd(void)
{
	...
	if (buffer)											// Si il n'y a aucune erreur de malloc
		ft_putstr_fd(buffer, 1);					   // On affiche le path actuel contenu dans le buffer 
	else											  // Sinon
	{
		write(2, RED, ft_strlen(RED));				// On ecrit en rouge sur la sortie erreur
		write(2, "minishell: pwd: ", 16);		   // minishell: pwd:
		ft_putstr_fd(strerror(errno), 2);		  // Suivi du retour de l'erreur gerer par errno
