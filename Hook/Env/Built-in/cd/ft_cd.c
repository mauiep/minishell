...
/*   Updated: 2022/11/10 12:50:26 by ceatgie          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_cd_else(t_mini *data)
{
	...
	data->converted_path = ft_convert_arg_to_path(data);			  // On converti l'argument en path
...
{
	if (data->cmd && data->cmd[1] && data->cmd[2])											  // Si il existe plus de 1 arguments (donc autre chose que le path ou on veut aller)
	{
		ft_putstr_fd(RED, 2);																// On ecrit en rouge sur la sortie erreur
		ft_putstr_fd("minishell: cd: too many arguments", 2);							   // minishell: cd: too many arguments
		ft_putstr_fd(RESET, 2);															  // On reset la couleur sur la sortie erreur
		return (1);																		 // On return (1);
	}
	else if (!data->cmd[1] || !ft_strcmp("~", data->cmd[1])							   // Sinon si il n'y a pas de path ou si l'argument est ~
		||	!ft_strcmp("--", data->cmd[1]))											  // Ou --
	{
		...
		{
			ft_putstr_fd(RED, 2);												 // On ecrit en rouge sur la sortie erreur
			ft_putstr_fd("minishell: cd: HOME not set", 2);						// minishell: cd: HOME not set
			ft_putstr_fd(RESET, 2);											   // On reset la couleur sur la sortie erreur
			return (1);														  // On return (1)
		}
		return (ft_cd_error_manager(data->home, data));						// Sinon on return (ft_cd_error_manager()) qui va s'occuper de changer de directory
	}
	return (ft_cd_else(data));											 // On return (ft_cd_else()) qui s'occupe de gerer le cas si l'argument vaut "-"
}